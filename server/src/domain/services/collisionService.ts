...
const collisionBullet = (bullet: Bullet, height: number, width: number): boolean => {
...
export const processCollisions = (
    ...
    for (const entity of entities) {
        collisionEntity(entity, heightRoom, widthRoom, hasDoors, areDoorsOpen);
    }

    for (const bullet of bullets) {
        if (bullet.isDestroyed) continue;
        if (collisionBullet(bullet, heightRoom, widthRoom)) continue;
