...
public processSnapshot(snapshot: GameSnapshotDTO): void {
    ...
    snapshot.players.forEach(p => {
        activeIds.add(p.id);
        this.updateOrCreate(p.id, p, p.hp, p.maxHp, p.mana, p.maxMana, p.gold, p.activeWeaponSprite, 'player');
    });
...
private updateOrCreate(
    ...
    gold: number, activeWeaponSprite: string, 
    ...
    entity.gold = gold; entity.activeWeaponSprite = activeWeaponSprite;
}
